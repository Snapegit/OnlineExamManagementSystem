						...
						<el-col :span="24">
							<el-form-item prop="tupian" label="图片">
								<uploads
									...
									:fileUrls="userForm.tupian?userForm.tupian:''" 
...
<script setup>
	...
	const updateSession = () => {
		userFormRef.value.validate((valid)=>{
			if(valid){
				if(userForm.value.tupian!=null){
					userForm.value.tupian = userForm.value.tupian.replace(new RegExp(context?.$config.url,"g"),"");
				}
				context?.$http({
					url: `${tableName}/update`,
					method: 'post',
